...
def main():
    ...
    parser.add_argument("--cam-poses", action="store_true",
                        help="Use cam_poses_{gl,cv}.npy (frame-0 cam as world origin) "
                             "instead of cam2world_{gl,cv}.npy (MuJoCo world)")
    ...
    if args.opencv:
        ...
        if flow_npy:
            ...
            opencv_flow_fallback = True
            print(f"  flow (opengl, will flip y/z at runtime): {flow.shape}")
    else:
        flow = load_first_flow(traj_dir)
        print(f"  flow (opengl): {flow.shape}")
    T, H, W, _ = flow.shape

    if args.rdf or opencv_flow_fallback:
        flow = (flow.astype(np.float32) * FLIP3).astype(np.float32)
        if opencv_flow_fallback:
            print("  applied OpenGL->OpenCV flip to flow (no *_cv.npy available)")
        else:
    ...
    if args.flip_x or args.flip_y or args.flip_z:
        sign = np.array([
            -1.0 if args.flip_x else 1.0,
            -1.0 if args.flip_y else 1.0,
            -1.0 if args.flip_z else 1.0,
        ], dtype=np.float32)
        flow = flow * sign
        axes = "".join(a for a, f in zip("xyz", [args.flip_x, args.flip_y, args.flip_z]) if f)
    ...
    if args.cam_poses:
        c2w_candidates = (["cam_poses_cv.npy"] if args.opencv
                          else ["cam_poses_gl.npy"])
    elif args.opencv:
        c2w_candidates = ["cam2world_cv.npy", "cam2world.npy", "cam_poses_cv.npy", "cam_poses.npy"]
    else:
        c2w_candidates = ["cam2world_gl.npy", "cam2world.npy", "cam_poses_gl.npy", "cam_poses.npy"]
    c2w_path = None
    ...
    pose_suffix = "_cv" if args.opencv else "_gl"
    pose_files = sorted(traj_dir.glob(f"pose_*{pose_suffix}.npy"))
    poses = {}
    for pf in pose_files:
        name = pf.stem.replace("pose_", "").removesuffix(pose_suffix)
        poses[name] = np.load(str(pf)).astype(np.float32)
    ...
    def render_frame(t: int) -> None:
        ...
        for name, pose_arr in poses.items():
            pose_world = cam2world_for_pose[t] @ pose_arr[t]
