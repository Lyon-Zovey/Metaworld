#!/usr/bin/env python3
"""
批量重新数采 datasets_500_fix  (v3)
策略: 50 task 并行，每 task 内按 --num-procs 拆成多子进程。
      flow_compress 全局信号量限制并发，避免 NVENC / CPU 排队。
"""
import argparse
import json
import os
...
# Global semaphore: limits concurrent flow_compress (NVENC) calls
_flow_sem: threading.Semaphore = None  # set in main()


def run(cmd, log_fh, env=None, retries=0):
    e = os.environ.copy()
    if env:
        e.update(env)
...
def steps34(cam_dir: Path, log_fh):
    """Step3 + Step4 for a single traj dir, with flow_compress gated by semaphore."""
    ok = run([PYTHON, str(SCRIPTS/"traj2sceneflow/convert_camera_depths.py"), str(cam_dir)], log_fh)
    if not ok:
        return False

    # Gate NVENC-heavy flow_compress with global semaphore
    with _flow_sem:
    ...
    ok = run([PYTHON, str(SCRIPTS/"traj2sceneflow/point_compress.py"),
              "--seg_dir", str(cam_dir)], log_fh, retries=3)
    if not ok:
        return False

    ok = run([PYTHON, str(SCRIPTS/"traj2sceneflow/seg_compress.py"),
              "compress", "--seg-dir", str(cam_dir)], log_fh, retries=3)
    return ok
...
def main():
    ...
    parser.add_argument("--flow-concurrency", type=int, default=32,
                        help="Max simultaneous flow_compress (NVENC) calls (default=32)")
    ...
    global _flow_sem
    _flow_sem = threading.Semaphore(args.flow_concurrency)
    ...
    print(f"flow并发上限  : {args.flow_concurrency}")
    ...
    with ThreadPoolExecutor(max_workers=len(tasks)) as ex:
        ...
        for fut in as_completed(futs):
            ...
            print(f"[{done_tasks:2d}/{len(tasks)} tasks]  {task_name}  done={done_cnt}  "
