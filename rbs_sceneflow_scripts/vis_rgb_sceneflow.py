...
camera frame and RGB stores a color rgb[t,i,j]. If the (u,v) index space is
consistent across the two, coloring the sceneflow point cloud by the SAME (i,j)
of the RGB image should produce a recognizable scene.

This script is meant to verify what a downstream dataloader needs to do when
the camera is upside-down. Try the flags below:

    # raw, no rotation: baseline alignment check
    python vis_rgb_sceneflow.py <traj_dir> --port 8093

    # downstream rotates ONLY RGB 180 (typical mistake): colors will be wrong
    python vis_rgb_sceneflow.py <traj_dir> --port 8093 --rot-rgb-180

    # downstream rotates BOTH RGB and flow's (H,W) by 180 + flips flow value
    # (this is what vis_sceneflow.py --flip-flow-yz --rot-z-180 implicitly
    # corresponds to in a paired pipeline): everything should align again.
...
def main():
    ...
    ap.add_argument("--rot-rgb-180", action="store_true",
                    help="Rotate RGB / depth by 180 degrees in the image plane "
                         "(np.rot90 k=2). Simulates fixing an upside-down camera "
                         "at the image level.")
    ap.add_argument("--rot-flow-hw-180", action="store_true",
                    help="Rotate sceneflow's (H,W) by 180 degrees AND negate "
                         "flow values (x,y,z)->(-x,y,-z). This is the matching "
                         "operation paired with --rot-rgb-180 for a consistent "
                         "image+3D rotation. Value flip combines --flip-flow-yz "
                         "(yz negate, RUB->RDF) and --rot-z-180 (xy negate).")
    ...
    ap.add_argument("--full", action="store_true",
                    help="Full end-to-end check: also load seg / target-obj mask / "
                         "obj_pose / mesh and apply the matching rotations. seg & "
                         "mask follow --rot-rgb-180 (np.rot90 on H,W); obj_pose "
                         "gets ROT_Z_180 left-multiplied iff --rot-flow-hw-180 is "
                         "set; mesh is rendered with that pose; intrinsics-fx/fy "
                         "and cam2world stay unchanged.")
    ...
    # ---- load everything in RAW form first ----
    ...
    # ---- apply requested rotations (image plane + flow plane) ----
    if args.rot_rgb_180:
        ...
        if target_mask is not None:
            target_mask = np.rot90(target_mask, k=2, axes=(1, 2)).copy()
        K = K.copy()
        K[0, 2] = W - 1 - K[0, 2]
        K[1, 2] = H - 1 - K[1, 2]
        print(f"  [rot-rgb-180] rotated RGB/depth/seg/mask; K_cx,cy -> ({K[0,2]:.2f}, {K[1,2]:.2f})")

    if args.rot_flow_hw_180:
        flow = np.rot90(flow, k=2, axes=(1, 2)).copy()
        flow = flow * np.array([-1.0, 1.0, -1.0], dtype=np.float32)
        for name in obj_poses:
            obj_poses[name] = (ROT_Z_180 @ obj_poses[name]).astype(np.float32)
        print("  [rot-flow-hw-180] rotated flow (H,W); values *= (-1, 1, -1); "
              f"left-multiplied {len(obj_poses)} obj_pose(s) by ROT_Z_180")
    else:
        # Default: convert OpenGL flow to OpenCV (matches --flip-flow-yz)
        flow = flow * FLIP3
        print("  [default] flow *= (1, -1, -1)  (OpenGL -> OpenCV cam frame)")
    ...
    # vertices are body-local; per meta.json: p_cam_h = T_body2cam[t] @ [v_body; 1]
    # then world = cam2world[t] @ p_cam_h.  obj_poses may already be left-
    # multiplied by ROT_Z_180 above to stay consistent with --rot-flow-hw-180.
    ...
    # Pick a deterministic trail subset per object (indices into vertex array).
    ...
    def render(t: int):
        ...
        # target_mask point cloud: only the flow points where mask>0, rendered in red
        ...
        # 2D target_mask image panel (so you can compare pixel layout)
        ...
        # obj_pose frames + meshes (in full mode)
        ...
        # ---- object flow: current-frame vertex cloud + optional trails ----
        ...
        if show_objflow_trails_cb.value and t > 0:
            for name, pts_TN3 in obj_flow_world.items():
                ...
                if idx is None or idx.size == 0:
                    continue
                # trail per selected vertex: (t, idx, 3) -> segments (t-1, idx, 2, 3)
    ...
    print("  - depth_rgb: depth back-projected with the (possibly rotated) K,")
    print("               also colored by rgb[i,j]. Should overlap flow_rgb.")
    print("  Mismatch (colors splattered, two clouds offset) means the image-plane")
    print("  rotation and the (u,v)/intrinsics handling are inconsistent.")
